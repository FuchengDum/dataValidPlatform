...
该目录是运行产物，迁移源码或制作干净分发包时可以不包含，演示前重新执行 CLI 会自动生成。
...
6. CLI 模式支持 `lint`、`run`、`validate` 和 `recommend`，可用于脚本化校验、分发包演示和 CI 质量门禁。
7. 当前版本默认使用 H2，后续可通过 profile 切换 MySQL，并扩展数据库表输入和 SQL 查询结果输入。
