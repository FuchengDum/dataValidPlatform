# CLI 使用 V3 业务表复现 Web 校验结果实施与验收记录

本文是 `通用数据验证工具后续演进计划.md` 中“阶段 14：CLI 使用 V3 业务表复现 Web 校验结果”的专项实施记录。后续路线、阶段状态、AI 配置和通用规则扩展方式以主演进计划为准；本文只保留 Case5/V3 复现链路的实现细节、关键决策和验收依据。
...
后续新增业务场景或新规则时，统一参考 `通用数据验证工具后续演进计划.md` 中阶段 14 的“新增规则免改代码方式”。本专项不再维护独立路线，避免与主演进计划出现口径分叉。
...
AI API Key、`LOCAL_AI_*` 环境变量和 `--debug-ai` prompt/response 落盘说明统一维护在 `通用数据验证工具后续演进计划.md` 的 AI 推荐章节。
...
- 本文不作为后续阶段路线来源，后续新增能力统一维护在 `通用数据验证工具后续演进计划.md`。
